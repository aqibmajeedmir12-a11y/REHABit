  ...
  window.habits.slice(0, 5).forEach(h => {
    ...
    el.innerHTML = `
      ...
      <div class="habit-info">
        ...
        <div class="habit-meta">${h.category} · ${h.time}</div>
      </div>
...
function drawProgressChart(days, mode){
  ...
  container.innerHTML = `
    <svg width="100%" height="${H}" viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" style="overflow:visible">
      ...
      <!-- Line -->
      <path d="${linePath}" fill="none" stroke="url(#lineGrad)" stroke-width="2.5" stroke-linecap="round"/>
...
function checkFreezeActivation(freeze, streak){
  ...
  if(streak === 0 && freeze.count > 0 && !freeze.activeUntil){
    ...
    fetch(`${API_BASE}/api/habits/analytics/logs`)
      .then(r => r.json())
      .then(data => {
